...
Options are:\n\
    ...
    -mode <mode>        Test mode, which can be:\n\
                        ...
                        mapnodetest   The same as mapnode but uses the test set\n\
                                  (added by Giuseppe Vettigli)\n\
...
Return value:
*****************************************************************************/
void MapNode(struct Map map, struct Graph *graph)
{
  UNSIGNED n;
  UNSIGNED x, y;
  struct Node *node;
...
void VisualizeGraph(struct Parameters parameters)
{
  ...
  vmap = GetHits(parameters.map.xdim, parameters.map.ydim, parameters.train, LEAF | INTERMEDIATE | ROOT | QUIET);
...
void AnalyseDataset(struct Parameters parameters)
{
  ...
  for (graph = parameters.train; graph != NULL; graph = graph->next){
    ...
    if (roots > 1 && lroots > 0 && roots != lroots){
      fprintf(stderr, "Graph %s has more than one root nodes where some of which are unlabelled.\n", graph->gname);
    }
    if (roots == 0){
      fprintf(stderr, "Graph %s has no root node.\n", graph->gname);
    }
  }
...
void ListDistances(struct Parameters parameters)
{
  ...
  for (graph = parameters.train; graph != NULL; graph = graph->next){
	...
	dist  = ComputeHexaDistance(25, 76, map->codes[nc].x, map->codes[nc].y);
	if (dist > 1.1 ||  dist < 0.9)
	  continue;
	printf("    %d %d \n", map->codes[nc].x, map->codes[nc].y);
      }
      exit(0);
...
int main(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++){
	...
	mode |= MAPGRAPH;
      else if (!strcmp(argv[i], "mapnode"))
	mode |= MAPNODE;
      else if (!strcmp(argv[i], "mapnodetest"))
	mode |= MAPNODETEST;
  ...
  if (CheckErrors() == 0){     /* If there were no errors so far then      */
    ...
    if (mode & MAPNODETEST)
      MapNode(parameters.map, parameters.test);
    if (mode & SHOWGRAPH)
